...
class CustomRequest(web_request.Request):
    ...
    def __init__(self, app: WebApplication, *args, **kwargs):
        ...
        self.host_without_port: str | None = (
            None if self.host is None else self.host.split(":", 1)[0]
        )
...
class HTTPStatus(IntEnum):
    ...
    NOT_ACCEPTABLE = (
        406,
        "Not Acceptable",
        """La ressource demandée n'est pas disponible dans un format qui respecterait l'en-tête "Accept" de la requête""",
    )
    ...
    REQUEST_TIMEOUT = (
        408,
        "Request Timeout",
        "Temps d’attente d’une requête du client, écoulé côté serveur",
    )
    ...
    UNSUPPORTED_MEDIA_TYPE = (
        ...
        "Format de requête non supporté pour une méthode et une ressource donnée",
    )
...
class CustomHTTPException(Exception):
    ...
    @classmethod
    def only_explain(
        cls, status: int | HTTPStatus, explain: str, headers=None, /
    ) -> CustomHTTPException:
