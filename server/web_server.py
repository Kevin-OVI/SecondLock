    ...
    def compile_format(self, log_format: str) -> tuple[str, list[web_log.KeyMethod]]:
        ...
        for atom in self.FORMAT_RE.findall(log_format):
            if atom[1] == "":
                ...
                key_method = web_log.KeyMethod(format_key2, functools.partial(m, atom[1]))
...
class WebRequestHandler(web_protocol.RequestHandler):
    def log_access(self, request: CustomRequest, response: web_response.StreamResponse, time: float) -> None:
        if request.log_request:
            super().log_access(request, response, time)


class WebApplication(web_server.Server, AutoLogger):
    def __init__(self, modules_manager: module_loader.ModulesManager, loop: asyncio.AbstractEventLoop):
        # noinspection PyTypeChecker
        super().__init__(self._handle, loop=loop)
        self.modules_manager = modules_manager

    def __call__(self) -> WebRequestHandler:
        return WebRequestHandler(self, loop=self._loop, access_log_class=WebAccessLogger, access_log_format=WebAccessLogger.LOG_FORMAT,
            max_line_size=16382)
    ...
    async def _handle(self, request: CustomRequest) -> web_response.StreamResponse:
        ...
        with self.modules_manager.requests_counter:
            request.site_host = self.modules_manager.special_module.get_sitehost(request)
            ...
            try:
                for pre_handler in self.modules_manager.pre_handlers:
                    ...
                    response = await self.modules_manager.special_module.handle_request(request)
            except web.HTTPException as e:
                if e.status < 400:
                    response = e
                else:
                    response = await self.modules_manager.special_module.create_exception_response(request,
                        CustomHTTPException(e.status, e.reason, e.text, e.headers))
            except CustomHTTPException as e:
                response = await self.modules_manager.special_module.create_exception_response(request, e)
            except Exception:
                traceback.print_exc()
                response = await self.modules_manager.special_module.create_exception_response(request, CustomHTTPException(HTTPStatus.INTERNAL_SERVER_ERROR))
            ...
            for pre_handler in reversed(pre_handlers_stack):
                ...
                try:
                    ...
                    self.logger.error(f"Error occured during execution of {pre_handler} handle_response method :\n{traceback.format_exc()}")
            return response

    def _make_request(self, message: web_request.RawRequestMessage, payload: web_request.StreamReader, protocol: web_protocol.RequestHandler,
