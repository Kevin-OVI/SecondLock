...
from ...utils import RateLimitChecker, basic_ip_ratelimit, ip_lock, make_json_response, parse_json_content
...
class APIAuthenticationModule(HTTPModule):
    ...
    async def post_login(self, request: CustomRequest) -> StreamResponse:
        ...
        res = self.core.db.execute("SELECT id, passhash FROM users WHERE username=?", (login_payload.username,)).fetchone()
        if res is None:
        ...
        return make_json_response(HTTPStatus.OK, {
            ...
            "token": self.core.token_encryptor_manager.generate_token(user_id, login_payload.password)
        })
    ...
    async def post_register(self, request: CustomRequest) -> StreamResponse:
        ...
        if self.core.db.execute("SELECT id FROM users WHERE username=?", (register_payload.username,)).fetchone() is not None:
        ...
        with self.core.db:
            try:
                user_id = self.core.db.execute("INSERT INTO users (username, passhash) VALUES (?, ?) RETURNING id",
        ...
        return make_json_response(HTTPStatus.CREATED, {
            ...
            "token": self.core.token_encryptor_manager.generate_token(user_id, register_payload.password)
        })

    @route("PATCH", "/api/user")
    async def patch_user(self, request: CustomRequest) -> StreamResponse:
        old_token, (old_username, old_passhash_db) = self.core.check_authorization_advanced(request)
        ...
        token_string, new_token = self.core.token_encryptor_manager.regenerate_token(old_token, update_user_payload.new_password)

        with self.core.db:
            try:
                self.core.db.execute("UPDATE users SET username=?, passhash=? WHERE id=?", (new_username, new_passhash_db, old_token.user_id))
            except sqlite3.IntegrityError:
                raise CustomHTTPException.only_explain(HTTPStatus.CONFLICT, "The 'username' is already used")
            if update_user_payload.new_password is not None:
                self.core.db.executemany("UPDATE sites SET name=?, secret=? WHERE id=?", (
                    (
                    ...
                    self.core.db.execute("SELECT id, name, secret FROM sites WHERE user=?", (old_token.user_id,))
                ))
