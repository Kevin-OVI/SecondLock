...
export default function PasswordStrengthBar({
  ...
  return (
    <div
      ...
      style={{ "--password-strength": strength } as CSSProperties}
    >
      <div className={styles.progressContainer}>
        <div className={styles.progress}></div>
        <div className={styles.progressText}>
          Mot de passe{" "}
          {strength < 0.4 ? "faible" : strength === 1 ? "fort" : "moyen"}
        </div>
      </div>
      ...
      <div>
        {checkedRules.map(({ label, passed }) => (
          <div className={styles.check} key={label}>
            {passed ? (
              <CheckIcon className={genericStyles.colorGreen} />
            ) : (
              <CloseIcon className={genericStyles.colorRed} />
            )}{" "}
            {label}
          </div>
        ))}
